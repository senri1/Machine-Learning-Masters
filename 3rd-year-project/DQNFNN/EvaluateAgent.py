...
def evaluateStateQvalues(agent):
    ...
    with torch.no_grad():
        ...
        s = agent.encoder(s)
        s = s.view(-1,400).detach().cpu().numpy()
        s = s - agent.mean
        s = np.divide(s, agent.sd, out=np.zeros_like(s), where=agent.sd!=0)
        q = agent.fnn(torch.from_numpy(s).float().to('cuda')).detach().cpu().numpy()
    q = np.mean(q,axis=1)
    q = np.mean(q,axis=0)
    return float(q)
